...
def run_model(testX: np.ndarray):
    ...
    model = keras.models.load_model(f"keras_models/000_entire_close.keras")
...
def main():
    ...
    for path in paths:
        ...
        prob = cal_correct_prob(path, result_df)

        csv_df.loc[idx] = [code, name, prob]
        print(csv_df.tail())
    print(csv_df)
    csv_df.to_csv("prob.csv", encoding="cp949")
